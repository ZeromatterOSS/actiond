...
run_smoke() {
  ...
  if [[ "${executor_timing_logs}" == "1" ]]; then
    "${repo_root}/test/parse_timings.py" "${measured_server_log}" \
      --mode "llvm-vm" \
      --command "ACTIOND_VM_CAS_IMAGE_SIZE_MIB=${cas_image_size_mib} ACTIOND_VM_MEMORY_MIB=${memory_mib} ACTIOND_VM_CPUS=${cpus} ACTIOND_LLVM_SMOKE_JOBS=${jobs} ACTIOND_VM_QEMU_CACHE=${ACTIOND_VM_QEMU_CACHE:-none} ACTIOND_VM_QEMU_AIO=${ACTIOND_VM_QEMU_AIO:-default} ACTIOND_LLVM_SMOKE_OPT_SERVER=${ACTIOND_LLVM_SMOKE_OPT_SERVER:-0} ACTIOND_LLVM_SMOKE_EXECUTOR_TIMING_LOGS=${executor_timing_logs} e2e/run_llvm_vm_smoke.sh" \
