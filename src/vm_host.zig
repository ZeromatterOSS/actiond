...
pub fn parseServeQemuVmArgs(args: []const []const u8) !ServeQemuVmOptions {
    var filtered: std.ArrayListUnmanaged([]const u8) = .empty;
    defer filtered.deinit(std.heap.smp_allocator);

    var options = ServeQemuVmOptions{};
    var i: usize = 0;
    while (i < args.len) {
        const arg = args[i];
        if (std.mem.eql(u8, arg, "--qemu")) {
            i += 1;
            if (i >= args.len) return error.MissingServeArgumentValue;
            ...
            i += 1;
            if (i >= args.len) return error.MissingServeArgumentValue;
            options.guest_cid = try parseU32(args[i]);
        } else if (std.mem.startsWith(u8, arg, "--guest-cid=")) {
            options.guest_cid = try parseU32(arg["--guest-cid=".len..]);
        } else if (std.mem.eql(u8, arg, "--allow-tcg")) {
            options.allow_tcg = true;
        } else if (std.mem.eql(u8, arg, "--qemu-cache")) {
            i += 1;
            if (i >= args.len) return error.MissingServeArgumentValue;
            options.qemu_cache = args[i];
        } else if (std.mem.startsWith(u8, arg, "--qemu-cache=")) {
            options.qemu_cache = arg["--qemu-cache=".len..];
        } else if (std.mem.eql(u8, arg, "--qemu-aio")) {
            i += 1;
            if (i >= args.len) return error.MissingServeArgumentValue;
    ...
    options.common = try parseServeVmArgs(filtered.items);
    return options;
}
...
pub fn serveDarwin(
    ...
    return serveWithMachine(io, allocator, options, .darwin, .{});
}

pub fn serveQemu(
    io: std.Io,
    allocator: std.mem.Allocator,
    options: ServeQemuVmOptions,
) !void {
    if (options.common.cas_image == null) return error.MissingCasImage;
    return serveWithMachine(io, allocator, options.common, .qemu, options);
}

const MachineKind = enum { darwin, qemu };

fn serveWithMachine(
    io: std.Io,
    allocator: std.mem.Allocator,
    options: ServeVmOptions,
    comptime machine_kind: MachineKind,
    qemu_options: anytype,
) !void {
    ...
    var vm = if (comptime machine_kind == .qemu)
        try qemu_vm.Machine.start(io, allocator, .{
    ...
    defer if (comptime machine_kind == .qemu) vm.deinit(io) else vm.deinit();
...
test "parseServeQemuVmArgs accepts QEMU flags" {
...
test "serveQemu requires an explicitly formatted CAS image" {
    try std.testing.expectError(error.MissingCasImage, serveQemu(std.testing.io, std.testing.allocator, .{
        .common = .{
            .kernel = "/tmp/bzImage",
